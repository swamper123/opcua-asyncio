...
class State(object):
    ...
    id_: "BaseVariableType" Id is a name which uniquely identifies the current state within the StateMachineType.
    ...
    def __init__(self, id_, name: str = None, number: int = None, node: Node = None):
        if id_ is not None:
            self.id = ua.Variant(id_)
        else:
            self.id = id_  # in this case it needs to be added with add_state which takes the nodeid returen from add_state
        self.name = name
        self.number = number
...
class Transition(object):
    ...
    id_: "BaseVariableType" Id is a name which uniquely identifies a Transition within the StateMachineType.
        A subtype may restrict the DataType.
    number: Number is an integer which uniquely identifies the current state within the StateMachineType.
    transitiontime: TransitionTime specifies when the transition occurred.
    effectivetransitiontime: EffectiveTransitionTime specifies the time when the current state
                             or one of its substates was entered.
    If, for example, a StateA is active and – while active – switches several times between its substates SubA and SubB,
    then the TransitionTime stays at the point in time where StateA became active whereas the
    ...
    def __init__(self, id_, name: str = None, number: int = None, node: Node = None):
        if id_ is not None:
            ...
            self.id = id_  # in this case it needs to be added with add_transition which takes the nodeid returen from add_transition
        self.name = name
        self.number = number
...
class StateMachine(object):
    ...
    async def init(self, statemachine: Node):
        ...
        for prop in current_state_props:
            ...
            if dn.Text == "Id":
                ...
                self._current_state_effective_display_name_node = await self._current_state_node.get_child(
        ...
        if self._optionals:
            ...
            for prop in last_transition_props:
                ...
                if dn.Text == "Id":
                    ...
                    self._last_transition_transitiontime_node = await self._last_transition_node.get_child(
                        ["TransitionTime"])
                else:
                    _logger.warning(
                        f"{await statemachine.read_browse_name()} LastTransition Unknown propertie: {dn.Text}")
        self._evgen = await self._server.get_event_generator(self.evtype, self._state_machine_node)

    async def change_state(self, state: State, transition: Transition = None,
                           event_msg: Union[str, ua.LocalizedText] = None, severity: int = 500):
        """
        ...
        if event_msg:
            ...
            if not isinstance(event_msg, ua.LocalizedText):
                raise ValueError(
                    f"Statemachine: {self._name} -> event_msg: {event_msg} is not a instance of LocalizedText")
    ...
    async def _write_state(self, state: State):
        ...
        if state.node:
            ...
            if self._current_state_effective_display_name_node and state.effectivedisplayname:
                await self._current_state_effective_display_name_node.write_value(state.effectivedisplayname,
                                                                                  ua.VariantType.LocalizedText)
    ...
    async def _write_transition(self, transition: Transition):
        ...
        if self._optionals:
            ...
            if self._last_transition_name_node and transition.name:
                await self._last_transition_name_node.write_value(ua.QualifiedName(transition.name, self._idx),
    ...
    async def add_state(self, state: State, state_type: ua.NodeId = ua.NodeId(2307, 0), optionals: bool = False):
        ...
        if state_type not in [ua.NodeId(2309, 0), ua.NodeId(2307, 0), ua.NodeId(15109, 0)]:
            # unknown state type!
            raise ValueError(
                f"Statemachine: {self._name} -> state_type: {state_type} "
                f"is not in list: [ua.NodeId(2309, 0),ua.NodeId(2307, 0),ua.NodeId(15109,0)]")
        if not state.name:
    ...
    async def add_transition(self, transition: Transition, transition_type: ua.NodeId = ua.NodeId(2310, 0),
    ...
    async def set_available_transitions(self, transitions: List[ua.NodeId]):
        if self._optionals:
            ...
            if isinstance(transitions, list) and all(isinstance(transition, ua.NodeId) for transition in transitions):
                return await self._available_transitions_node.write_value(transitions,
...
class ExclusiveLimitStateMachine(FiniteStateMachine):
    ...
    def __init__(self, server=None, parent=None, idx=None, name=None):
        super().__init__(server, parent, idx, name)
        if name is None:
            self._name = "ExclusiveLimitStateMachine"
...
class FileTransferStateMachine(FiniteStateMachine):
    ...
    def __init__(self, server=None, parent=None, idx=None, name=None):
        super().__init__(server, parent, idx, name)
        if name is None:
            self._name = "FileTransferStateMachine"
...
class ShelvedStateMachine(FiniteStateMachine):
    ...
    def __init__(self, server=None, parent=None, idx=None, name=None):
        super().__init__(server, parent, idx, name)
        if name is None:
            self._name = "ShelvedStateMachine"
